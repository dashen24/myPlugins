...
$.fn.drag = function(s) {
    ...
    return this.each(function(){
        ...
        start = function(e){
            ...
            fxd = 'fixed'===$t.css('position')? 1: 0;
            
            if(s.scroll){
                ox = -e.pageX - s.scroll.scrollLeft();
                oy = -e.pageY - s.scroll.scrollTop();
            }else{
                x = $t.offset();//获得相对位置
                if(s.size){
                    w = $t.width();//记录高度和宽度
                    ...
                    if(s.proxy){
                        ...
                        ox = e.pageX - x.left + win.scrollLeft();
                        oy = e.pageY - x.top + win.scrollTop();
                    }
                }
                if(s.proxy){
                    var rw = $t.outerWidth(), rh = $t.outerHeight();
                    prx = $('<div style="position:absolute;width:'+ rw +'px;height:'+ rh +'px;z-index:99999;"></div>').css($t.position());
                    morg = prx;
                    pw = rw - w;
                    ph = rh - h;
                }else{
            ...
            doc.bind('mousemove.' + id, move).bind('mouseup.'+ id, stop);
            if(hd[0].setCapture){
        ...
        move = function(e){
            ...
            if(1 === dr){//第一次进来
                ...
                if(!s.scroll){
                    ...
                    drop = [];
                    $('*[jquerydd]', s.region||doc).each(function(){
            ...
            e.drag = drag;
            if(s.scroll){
                var f = h - s.scroll.height(), o = -e.pageY - oy;
                if(f>0){
                    if(o<0){
                        o = 0;
                    }
                ...
                if(s.size){//改变宽度和高度
                    var sw = e.pageX - ow, sh = e.pageY - oh;
                    //限定宽度和高度
                    ...
                    var sx = e.pageX - ox, sy = e.pageY - oy;
                    if(s.region){//判断是否在指定的Region里面
                        if(sx < rleft){
                            sx = rleft;
                ...
                if(drag){
                    ...
                    if(s.mousehit){
                        ax = e.pageX - doc.scrollTop();
                        ay = e.pageY - doc.scrollLeft();
                        aw = 1;
                        ah = 1;
                    }else{
                        da = drag[0].offset();
                    ...
                    for(i=0,j=drop.length; i<j; i++){
                        ...
                        if(dx.is(':visible') && dx[0]!==t){
                            ...
                            bx = db.left, by = db.top, bw = dx.outerWidth(), bh = dx.outerHeight();
